...
import { Download, RotateCcw, RotateCw, ArrowUp, ArrowDown, ZoomIn, ZoomOut } from "lucide-react"
...
export default function ModelViewer({ imageUrl }: ModelViewerProps) {
  ...
  const [cameraPos, setCameraPos] = useState<[number, number, number]>([0, 0, 2])
  ...
  const rotateLeft = () => {
    if (modelGroupRef.current) {
      modelGroupRef.current.rotation.y += Math.PI / 12
    }
  }
  ...
  const rotateUp = () => {
    if (modelGroupRef.current) {
      modelGroupRef.current.rotation.x = Math.max(-Math.PI / 2, modelGroupRef.current.rotation.x - Math.PI / 18)
    }
  }

  const rotateDown = () => {
    if (modelGroupRef.current) {
      modelGroupRef.current.rotation.x = Math.min(Math.PI / 2, modelGroupRef.current.rotation.x + Math.PI / 18)
    }
  }

  const zoomIn = () => {
    setCameraPos(([x, y, z]) => [x, y, Math.max(0.4, z - 0.4)])
    if (orbitRef.current) orbitRef.current.update()
  }

  const zoomOut = () => {
    setCameraPos(([x, y, z]) => [x, y, Math.min(10, z + 0.6)])
    if (orbitRef.current) orbitRef.current.update()
  }
  ...
  return (
    <div className="relative h-full flex flex-col">
      ...
      <div className="flex-1 min-h-[400px] bg-card/40 rounded-md overflow-hidden">
        {imageUrl ? (
          <Canvas shadows camera={{ position: cameraPos, fov: 50 }}>
            <PerspectiveCamera makeDefault position={cameraPos} ref={cameraRef} />
            ...
            <OrbitControls ref={orbitRef} autoRotate autoRotateSpeed={0.5} />
          </Canvas>
        ) : (
      ...
      <div className="mt-4 flex flex-col items-center gap-3">
        <div className="flex justify-center">
          <div className="flex bg-card p-1 rounded-md">
          <Button
        ...
        <div className="flex gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={rotateUp} title="Rotate up">
            <ArrowUp className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={rotateDown} title="Rotate down">
